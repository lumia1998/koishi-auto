...
async function decodeMessage(bot, data, message, payload = message, details = true) {
  ...
  if (data.sticker_items) {
    message.content += data.sticker_items.map((s) => (0, import_core2.h)("sticker", {
      id: s.id,
      format_type: s.format_type,
      name: s.name
    }, [
      import_core2.h.image(`https://media.discordapp.net/stickers/${s.id}.webp?size=160`)
    ])).join("");
  }
...
var decodeArgv = /* @__PURE__ */ __name((data, command) => {
  ...
  for (const argument of command.arguments) {
    ...
    if (value !== void 0) result.arguments.push(value);
  }
  for (const option of command.options) {
    const name = option.name.toLowerCase();
    const value = options.find((opt) => opt.name === name)?.value;
    if (value !== void 0) result.options[option.name] = value;
  }
  return result;
}, "decodeArgv");
...
var DiscordMessageEncoder = class extends import_core3.MessageEncoder {
  ...
  async post(data, headers) {
    try {
      ...
      Object.defineProperty(session.event.message, "channel", {
        configurable: true,
        get: /* @__PURE__ */ __name(() => session.event.channel, "get")
      });
  ...
  decodeButton(attrs, label) {
    let style = 1 /* PRIMARY */;
    if (attrs.class === "secondary") style = 2 /* SECONDARY */;
    if (attrs.class === "danger") style = 4 /* DANGER */;
    if (attrs.class === "success") style = 3 /* SUCCESS */;
    if (attrs.type === "link") {
      return {
        type: 2 /* BUTTON */,
        url: attrs.href,
