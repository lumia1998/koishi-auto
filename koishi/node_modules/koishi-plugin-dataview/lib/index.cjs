...
function serialize(obj) {
  if (import_koishi.Binary.is(obj)) return `"b${obj.byteLength}"`;
  if (obj instanceof Date) return `"d${obj.toJSON()}"`;
  return JSON.stringify(obj, (_, value) => {
    if (import_koishi.Binary.is(value)) return `b${value.byteLength}`;
...
function deserialize(str) {
  ...
  return JSON.parse(
    str,
    (_, v) => typeof v === "string" ? v[0] === "s" ? v.slice(1) : v[0] === "b" ? void 0 : v[0] === "n" ? BigInt(v.slice(1)) : new Date(v.slice(1)) : v
  );
}
  ...
  async getInfo() {
    ...
    await Promise.all(Object.keys(this.ctx.model.tables).map(async (name) => {
      ...
      for (const [key, field] of Object.entries(result.tables[name].fields)) {
        if (!import_koishi2.Field.available(field)) delete result.tables[name].fields[key];
      }
...
var index_default = DatabaseProvider;
