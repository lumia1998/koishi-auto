...
var PackageProvider = class extends import_console.DataService {
  ...
  async parseExports(name) {
    try {
      const exports2 = await this.ctx.loader.resolve(name);
      const result = {};
      result.schema = exports2?.Config || exports2?.schema;
      result.usage = exports2?.usage;
      result.filter = exports2?.filter;
      const inject = exports2?.using || exports2?.inject || [];
      ...
      const runtime = this.ctx.registry.get(exports2);
