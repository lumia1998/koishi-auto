...
// src/index.ts
import { Logger as Logger3, Time as Time3 } from "koishi";
...
var ChatLunaPluginChain = class _ChatLunaPluginChain extends ChatLunaLLMChainWrapper2 {
  ...
  async call({
    ...
    response.message = new AIMessage2(response.output);
    return response;
  }
  get model() {
    return this.llm;
...
async function defaultFactory(ctx, service) {
  ...
  ctx.on("chatluna/model-removed", (service2, platform) => {
    ...
    wrapper.getCachedConversations().filter(
      ([_, conversation]) => conversation.room && parseRawModelName(conversation.room.model)[0] === platform
...
async function queryJoinedConversationRoom(ctx, session, name2) {
  ...
  if (userRoomInfoList.length > 1) {
    throw new ChatLunaError5(
      ChatLunaErrorCode5.UNKNOWN_ERROR,
...
async function checkConversationRoomAvailability(ctx, room) {
  ...
  const [platformName, modelName] = parseRawModelName2(room.model);
...
async function getTemplateConversationRoom(ctx, config) {
  ...
  const selectModelAndPreset = /* @__PURE__ */ __name(async () => {
    if (config.defaultModel === "无" || config.defaultModel == null) {
      ...
      const [platformName, modelName] = parseRawModelName2(
        config.defaultModel
      );
  ...
  if (config.defaultChatMode == null || config.defaultModel === "无" || config.defaultModel == null || config.defaultPreset == null) {
    if (config.defaultChatMode == null) {
      throw new ChatLunaError5(ChatLunaErrorCode5.ROOM_TEMPLATE_INVALID);
    }
...
async function transferConversationRoom(ctx, session, room, userId) {
  ...
  if (memberList.length === 0) {
    throw new ChatLunaError5(ChatLunaErrorCode5.ROOM_NOT_FOUND);
  }
  ...
  if (oldMaster.length === 1) {
    ...
    throw new ChatLunaError5(ChatLunaErrorCode5.ROOM_NOT_FOUND_MASTER);
  }
  await ctx.database.upsert("chathub_room_member", [
...
async function switchConversationRoom(ctx, session, id) {
  ...
  if (joinedRoom.length > 1) {
    throw new ChatLunaError5(
      ChatLunaErrorCode5.THE_NAME_FIND_IN_MULTIPLE_ROOMS
    );
...
async function muteUserFromConversationRoom(ctx, session, roomId, userId) {
  ...
  if (memberList.length === 0) {
    throw new ChatLunaError5(ChatLunaErrorCode5.ROOM_NOT_JOINED);
  }
  await ctx.database.upsert("chathub_room_member", [
...
function checkModelList(service, models) {
  const availableModels = service.listAllModels(ModelType2.llm).value.map((model) => model.toModelName());
...
function checkModelList2(service, models) {
  const availableModels = service.listAllModels(ModelType3.llm).value.map((model) => model.toModelName());
  return models.some((model) => !availableModels.includes(model));
}
...
function apply22(ctx, config, chain) {
  ...
  async function oldChatLimitCheck(session, context) {
    ...
    if ((config.defaultModel === "无" || config.defaultModel.trim().length < 1) && ctx.chatluna.platform.listAllModels(ModelType4.all).value.length < 1) {
...
function apply23(ctx, config, chain) {
  ...
  async function oldChatLimitSave(session, context) {
    ...
    let key = conversationId + "-" + session.userId;
    key = createHash2("md5").update(key).digest("hex");
    chatLimit.count++;
...
var StreamingBufferText = class {
  ...
  processChunk(chunk) {
    ...
    if (typeof content === "string") {
      elements.push(h6.text(content));
    } else if (Array.isArray(content)) {
      for (const item of content) {
        if (item.type === "text" && item.text) {
          elements.push(h6.text(item.text));
    ...
    return this.createProcessor("markdown", {
            ...
            for (const char of text) {
              ...
              if (!inContent) {
                ...
                if (this.postfix && bufferText.endsWith(this.postfix)) {
                  ...
                  if (content.trim()) {
                    controller.enqueue(h6.text(content));
                  }
                  return;
                }
                let searchIndex = 0;
      ...
      flush: /* @__PURE__ */ __name((controller) => {
        if (inContent && bufferText.trim()) {
          controller.enqueue(h6.text(bufferText));
        }
  ...
  processTextContent(state, text) {
    if (!state.inContent) {
      if (this.prefix && text.includes(this.prefix)) {
        ...
        if (afterPrefix) {
          this.mergeTextElement(state.elements, h6.text(afterPrefix));
        }
      }
      return false;
    }
    if (this.postfix && text.includes(this.postfix)) {
      ...
      if (beforePostfix) {
        this.mergeTextElement(state.elements, h6.text(beforePostfix));
      }
      return true;
    }
    this.mergeTextElement(state.elements, h6.text(text));
    return false;
  }
  addEndText(elements, endText) {
    if (!endText) return [...elements];
    const result = [...elements];
    result.push(h6.text(endText));
    return result;
  }
  mergeTextElement(elements, newElement) {
    if (newElement.type === "text" && elements.length > 0) {
      ...
      if (lastElement.type === "text") {
        ...
        elements[elements.length - 1] = h6.text(lastText + newText);
        return;
      }
    }
  ...
  chain.middleware("request_model", async (session, context) => {
    ...
    if (presetTemplate == null) {
      throw new ChatLunaError6(
        ChatLunaErrorCode6.PRESET_NOT_FOUND,
...
function createRequestId(session, room, requestId = randomUUID2()) {
  const userKey = session.userId + "-" + (session.guildId ?? "") + "-" + room.conversationId;
...
function createTokenCountHandler(context, session, config) {
  return async (tokens) => {
    ...
    const balance = await context.ctx.chatluna_auth.calculateBalance(
      session,
      parseRawModelName4(context.options.room.model)[0],
      tokens
    );
...
function apply33(ctx, config, chain) {
  ...
  chain.middleware("list_all_embeddings", async (session, context) => {
    ...
    const models = service.listAllModels(ModelType5.embeddings).value.map((model) => model.toModelName());
...
function apply34(ctx, config, chain) {
  ...
  chain.middleware("list_all_model", async (session, context) => {
    ...
    const models = services.listAllModels(ModelType6.llm).value.map((model) => model.toModelName());
    await pagination.push(models);
    context.message = await pagination.getFormattedPage(page, limit);
    return 1 /* STOP */;
  }).after("lifecycle-handle_command").before("lifecycle-request_model");
...
function apply38(ctx, config, chain) {
  ...
  chain.middleware("search_model", async (session, context) => {
    ...
    const models = services.listAllModels(ModelType7.llm).value.map((model) => model.toModelName());
    await pagination.push(models);
...
import { parseRawModelName as parseRawModelName5 } from "koishi-plugin-chatluna/llm-core/utils/count_tokens";
function apply39(ctx, config, chain) {
  ...
  chain.middleware("set_default_embeddings", async (session, context) => {
    ...
    const embeddings = service.listAllModels(ModelType8.embeddings);
    const [, modelName] = parseRawModelName5(setEmbeddings);
...
import { parseRawModelName as parseRawModelName6 } from "koishi-plugin-chatluna/llm-core/utils/count_tokens";
  ...
  chain.middleware("test_model", async (session, context) => {
    ...
    try {
      if (!model.includes("/")) {
        ...
        ;
        [platformName, modelName] = parseRawModelName6(model);
...
function apply43(ctx, config, chain) {
  chain.middleware("clone_preset", async (session, context) => {
    ...
    const loaded = load(newPreset.value.rawText);
...
function apply49(ctx, config, chain) {
  ...
  chain.middleware("create_room", async (session, context) => {
    ...
    const allModel = service.listAllModels(ModelType10.llm);
    if (name2 == null) {
      await context.send(session.text(".enter_name"));
      const result = await session.prompt(1e3 * 30);
      if (result == null) {
...
import { Pagination as Pagination8 } from "koishi-plugin-chatluna/utils/pagination";
function apply55(ctx, config, chain) {
  const pagination = new Pagination8({
    formatItem: /* @__PURE__ */ __name((value) => "", "formatItem"),
    formatString: {
      top: "",
      bottom: "",
...
function apply56(ctx, config, chain) {
  ...
  chain.middleware("resolve_room", async (session, context) => {
    ...
    if (config.allowChatWithRoomName) {
      ...
      const messageContent = typeof rawMessageContent === "string" ? rawMessageContent : h7.select(rawMessageContent, "text").join("").trimStart();
    ...
    if (joinRoom == null && (context.command?.length ?? 0) < 1) {
      if ((config.defaultModel === "无" || (config.defaultModel?.trim()?.length || 0) < 1) && ctx.chatluna.platform.listAllModels(ModelType11.llm).value.length < 1) {
        return session.text("chatluna.not_available_model");
      }
      ...
      cloneRoom.conversationId = randomUUID4();
...
import { ModelType as ModelType12 } from "koishi-plugin-chatluna/llm-core/platform/types";
function apply60(ctx, config, chain) {
  ...
  chain.middleware("set_room", async (session, context) => {
    ...
    while (true) {
      ...
      const findModel = service.listAllModels(ModelType12.llm).value.find(
...
async function checkRoomAvailability(context, session, ctx, room) {
  ...
  const findModel = ctx.chatluna.platform.listAllModels(ModelType12.llm).value.find((searchModel) => searchModel.toModelName() === room.model);
  if (findModel == null) {
...
// src/config.ts
import { Schema, Time as Time2 } from "koishi";
var Config2 = Schema.intersect([
  ...
  Schema.object({
    ...
    randomReplyFrequency: Schema.percent().min(0).max(1).step(0.01).default(0).computed()
  }),
  Schema.object({
    sendThinkingMessage: Schema.boolean().default(true),
    sendThinkingMessageTimeout: Schema.number().default(15e3),
    msgCooldown: Schema.number().min(0).max(3600).step(1).default(0),
    messageQueue: Schema.boolean().default(true),
    messageQueueDelay: Schema.number().min(0).max(60 * 30).default(0),
  ...
  Schema.object({
    blackList: Schema.number().min(0).max(1).step(1).default(0).computed().default(0)
  }),
  Schema.object({
    infiniteContext: Schema.boolean().default(true),
    autoDelete: Schema.boolean().default(false),
    autoDeleteTimeout: Schema.number().default(Time2.day * 10 / Time2.second).min(Time2.hour / Time2.second)
  }),
  Schema.object({
...
// src/render.ts
import { h as h14, Schema as Schema8 } from "koishi";
import {
...
var TextRenderer = class extends Renderer {
  ...
  async render(message, options) {
    ...
    if (transformed[0]?.type === "p") {
      ...
      if (pElementContent) {
        transformed.unshift(h8.text(pElementContent));
      } else {
  ...
  schema = Schema2.const("text").i18n({
...
var VoiceRenderer = class extends Renderer {
  ...
  schema = Schema3.const("voice").i18n({
    "zh-CN": "将回复渲染为语音",
...
// src/renders/raw.ts
import { transformMessageContentToElements as transformMessageContentToElements3 } from "koishi-plugin-chatluna/utils/string";
import { h as h10, Schema as Schema4 } from "koishi";
var RawRenderer = class extends Renderer {
  ...
  async render(message, options) {
    if (typeof message.content === "string") {
      return {
        element: h10.text(message.content)
      };
    }
    return {
      element: transformMessageContentToElements3(message.content)
    };
  }
  schema = Schema4.const("raw").i18n({
    "zh-CN": "原始输出",
    "en-US": "Raw text"
  });
};
...
var KoishiElementRenderer = class extends Renderer {
  ...
  async render(message, options) {
    let transformed = transformMessageContentToElements4(message.content);
  ...
  schema = Schema5.const("koishi-element").i18n({
...
function transformAndEscape2(source) {
  return source.flatMap((element) => {
    ...
    try {
      return h11.parse(element.attrs["content"]).map(unescape2);
    } catch (e) {
      logger14.error(e);
...
var MixedVoiceRenderer = class extends Renderer {
  ...
  async render(message, options) {
    const elements = [];
    const baseElements = transformMessageContentToElements5(message.content);
  ...
  _splitMessage(messages) {
    return messages.flatMap((message) => {
      ...
      const tokens = renderTokens2(
        marked2.lexer(message.attrs["content"])
      );
      if (tokens.length === 0 || tokens[0].length === 0) {
        return message.attrs["content"];
      }
  ...
  schema = Schema6.const("mixed-voice").i18n({
    "zh-CN": "同时输出语音和文本",
...
import { h as h13, Schema as Schema7 } from "koishi";
...
// src/renders/pure-text.ts
import he3 from "he";
import { transformMessageContentToElements as transformMessageContentToElements6 } from "koishi-plugin-chatluna/utils/string";
var PureTextRenderer = class extends Renderer {
  static {
    __name(this, "PureTextRenderer");
  }
  async render(message, options) {
    let transformed = transformMessageContentToElements6(message.content);
    if (options.split) {
    ...
    transformed = transformed.map((element) => {
      ...
      const content = element.attrs["content"];
      return h13.text(stripMarkdown(content));
    });
    return {
      element: transformed
    };
  }
  schema = Schema7.const("pure-text").i18n({
...
var DefaultRenderer = class {
  ...
  updateSchema() {
    ...
    this.ctx.schema.set(
      "output-mode",
      Schema8.union(this._getAllRendererScheme())
    );
  }
