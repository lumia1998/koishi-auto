...
var MessageTransformer = class {
  ...
  async transform(session, elements, model, message = {
    ...
    const sourceElementString = elements.map((h9) => h9.toString(true)).join();
    const quoteElementString = ((session.quote && session.quote.elements) ?? []).map((h9) => h9.toString(true)).join();
...
var TextRenderer = class extends Renderer {
  ...
  async render(message, options) {
    let transformed = (0, import_string2.transformMessageContentToElements)(message.content);
...
var VoiceRenderer = class extends Renderer {
  ...
  async render(message, options) {
    const baseElements = (0, import_string3.transformMessageContentToElements)(message.content);
...
var RawRenderer = class extends Renderer {
  ...
  async render(message, options) {
    ...
    return {
      element: (0, import_string4.transformMessageContentToElements)(message.content)
    };
  }
...
var KoishiElementRenderer = class extends Renderer {
  ...
  async render(message, options) {
    let transformed = (0, import_string5.transformMessageContentToElements)(message.content);
...
function transformAndEscape2(source) {
  return source.flatMap((element) => {
    ...
    try {
      return import_koishi6.h.parse(element.attrs["content"]).map(unescape2);
    } catch (e) {
...
var MixedVoiceRenderer = class extends Renderer {
  ...
  async render(message, options) {
    const elements = [];
    const baseElements = (0, import_string6.transformMessageContentToElements)(message.content);
...
var ChatLunaPromptRenderService = class {
  ...
  _initBuiltinFunctions() {
    ...
    this.registerFunctionProvider("timeDiff", (args) => {
      return (0, import_string8.getTimeDiff)(args[0], args[1]);
    });
    ...
    this.registerFunctionProvider("random", (args) => {
      ...
      return (0, import_string8.selectFromList)(args.join(","), false);
    });
    this.registerFunctionProvider("pick", (args) => {
      return (0, import_string8.selectFromList)(args.join(","), true);
    });
    this.registerFunctionProvider("roll", (args) => {
      return (0, import_string8.rollDice)(args[0]).toString();
    });
    this.registerFunctionProvider("url", async (args) => {
      return await (0, import_string8.fetchUrl)(
        args[1],
        args[0],
        args[2],
    ...
    return await Promise.all(
      messages.map(async (message) => {
        const content = await this.renderTemplate(
          (0, import_string8.getMessageContent)(message.content),
          variables,
          options
        );
...
var ChatLunaService = class extends import_koishi11.Service {
  ...
  async awaitLoadPlatform(plugin, timeout = 3e4) {
    ...
    this[import_koishi11.Context.origin].effect(() => () => disposable.stop());
    return promise;
  }
...
((ChatLunaPlugin2) => {
  ChatLunaPlugin2.Config = import_koishi11.Schema.intersect([
    import_koishi11.Schema.object({
      ...
      chatTimeLimit: import_koishi11.Schema.computed(
        import_koishi11.Schema.number().min(1).max(2e3)
      ).default(200),
    ...
    import_koishi11.Schema.union([
      ...
      import_koishi11.Schema.object({
        proxyMode: import_koishi11.Schema.const("off").required()
      }),
      import_koishi11.Schema.object({
        proxyMode: import_koishi11.Schema.const("system")
      })
    ])
  ]).i18n({
