...
var ChatLunaChatPrompt = class _ChatLunaChatPrompt extends import_prompts.BaseChatPromptTemplate {
  ...
  async _formatSystemPrompts(variables, configurable = {}) {
    const preset = this.preset.value;
    if (!this._tempPreset || this._tempPreset[0] !== preset) {
      this.conversationSummaryPrompt = import_prompts.HumanMessagePromptTemplate.fromTemplate(
...
Your goal is to provide better assistance based on these materials while maintaining natural and coherent responses.
  ...
  async formatMessages({
    ...
    const [formatAuthorsNote, usedTokensAuthorsNote] = authorsNote && (authorsNote.content?.length ?? 0) > 0 ? await this._counterAuthorsNote(
      authorsNote,
      variables,
      configurable
    ) : [null, 0];
  ...
  async _formatLoreBooks(loreBooks, usedTokens, result, variables) {
    ...
    for (const [position, array] of Object.entries(canUseLoreBooks)) {
      const message = await this.promptRenderService.renderMessages(
  ...
  async _formatWithMessagesPlaceholder(chatHistory, documents, usedTokens) {
    ...
    const availableLimit = this.sendTokenLimit - (documents.length > 0 ? 480 : 80);
  ...
  _buildConversationRounds(messages) {
    const rounds = [];
    let current = [];
  ...
  async _countMessagesTokens(messages) {
    let total = 0;
    for (const message of messages) {
  ...
  async _counterAuthorsNote(authorsNote, variables, configurable) {
    const formatAuthorsNote = await this.promptRenderService.renderTemplate(authorsNote.content, variables, {
  ...
  _formatAuthorsNote(authorsNote, result, [formatAuthorsNote, usedTokens]) {
    ...
    if (rawPosition === "in_chat") {
      result.splice(
        ...
        0,
        new import_messages.HumanMessage(formatAuthorsNote)
      );
    } else {
      result.splice(
        insertPosition,
  ...
  _findMessageIndex(chatHistory, insertPosition) {
