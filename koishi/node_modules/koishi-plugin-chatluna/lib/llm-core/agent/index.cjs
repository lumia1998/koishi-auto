...
Passed input: ${text}`
  ...
  parseAIMessage(message) {
    ...
    if (message.additional_kwargs.tool_calls && Array.isArray(message.additional_kwargs.tool_calls) && message.additional_kwargs.tool_calls.length > 0) {
    ...
    } else if ((message instanceof import_messages.AIMessageChunk || message instanceof import_messages.AIMessage) && message.tool_calls != null) {
...
function _convertAgentStepToMessages(action, observation) {
  if (isToolsAgentAction(action) && action.toolCallId !== void 0) {
    ...
    return action.messageLog?.concat(
      new import_messages2.FunctionMessage((0, import_string.getMessageContent)(observation), action.tool)
    );
  } else {
...
function createOpenAIAgent({
  ...
  const agent = import_runnables.RunnableSequence.from([
    ...
    import_runnables.RunnableLambda.from((input) => {
      if ((input?.additional_kwargs?.tool_calls && input?.additional_kwargs?.tool_calls.length > 0 || (input instanceof import_messages2.AIMessageChunk || input instanceof import_messages2.AIMessage) && input.tool_calls && input.tool_calls.length > 0) && outputParser instanceof OpenAIFunctionsAgentOutputParser) {
      ...
      return outputParser.parseResult([
        {
          message: input,
          text: (0, import_string.getMessageContent)(input.content)
        }
      ]);
    })
  ]);
...
var AgentExecutor = class _AgentExecutor extends import_base.BaseChain {
  ...
  handleToolRuntimeErrors = /* @__PURE__ */ __name((e) => "Something went wrong. Please Try Again. " + String(e), "handleToolRuntimeErrors");
  get inputKeys() {
  ...
  constructor(input) {
    ...
    this.handleToolRuntimeErrors = input.handleToolRuntimeErrors ?? this.handleToolRuntimeErrors;
...
function createToolsRef(options) {
  ...
  const getActiveTools = /* @__PURE__ */ __name((session, messages) => {
  ...
  const update = /* @__PURE__ */ __name((session, messages) => {
