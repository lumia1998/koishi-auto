...
var GrepTool = class extends import_tools4.StructuredTool {
  static {
  ...
  schema = import_zod3.default.object({
    pattern: import_zod3.default.string().describe("The regex pattern to search for."),
    searchPath: import_zod3.default.string().describe("The directory path to search in."),
    globPattern: import_zod3.default.string().optional().describe("Optional glob pattern to filter files."),
    outputMode: import_zod3.default.enum(["content", "files_with_matches", "count"]).optional().default("content").describe(
...
var GlobTool = class extends import_tools4.StructuredTool {
  static {
  ...
  schema = import_zod3.default.object({
    pattern: import_zod3.default.string().describe("The glob pattern to match files against."),
    searchPath: import_zod3.default.string().describe("The directory path to search in.")
  });
  store;
  constructor({ store, ...rest }) {
...
var RenameTool = class extends import_tools4.StructuredTool {
  static {
  ...
  schema = import_zod3.default.object({
    oldPath: import_zod3.default.string().describe("The current file or directory path."),
    newPath: import_zod3.default.string().describe("The new file or directory path.")
  });
  store;
  constructor({ store, ...rest }) {
    super(rest);
...
var MultiRenameTool = class extends import_tools4.StructuredTool {
  static {
  ...
  schema = import_zod3.default.object({
    pattern: import_zod3.default.string().describe("The glob pattern to match files."),
    replacement: import_zod3.default.string().describe("The new name pattern."),
    searchPath: import_zod3.default.string().optional().describe(
...
var MultiWriteFileTool = class extends import_tools4.StructuredTool {
  static {
  ...
  schema = import_zod3.default.object({
    files: import_zod3.default.array(
...
var UpdateFileTool = class extends import_tools4.StructuredTool {
  static {
  ...
  schema = import_zod3.default.object({
    filePath: import_zod3.default.string().describe("The path to the file to update."),
    oldString: import_zod3.default.string().describe("The text string to find and replace."),
    newString: import_zod3.default.string().describe("The replacement text string."),
    replaceCount: import_zod3.default.number().optional().describe(
...
__name(apply5, "apply");
var GroupMuteTool = class extends import_tools5.StructuredTool {
  constructor(config) {
    super({});
    this.config = config;
  }
  ...
  schema = import_zod4.z.object({
    userId: import_zod4.z.string().describe("The user ID to mute"),
    muteTime: import_zod4.z.number().describe(
      "Mute duration in seconds, minimum 1 seconds, 0 to unmute"
    ),
    operatorUserId: import_zod4.z.string().optional().describe(
...
async function apply6(ctx, config, plugin2) {
  ...
  plugin2.registerTool("music", {
    selector(history) {
      ...
      return history.some(
        (message) => message.content != null && (0, import_string5.fuzzyQuery)(
          (0, import_string5.getMessageContent)(message.content),
...
__name(apply6, "apply");
var MusicTool = class extends import_tools6.Tool {
  static {
...
async function apply7(ctx, config, plugin2) {
  ...
  for (const [operation, path2, spec, action] of parsedActions) {
    ...
    plugin2.registerTool(tool.name, {
      selector(history) {
        return history.some((item) => {
          const content = (0, import_string6.getMessageContent)(item.content);
          return (0, import_string6.fuzzyQuery)(content, [
            "令",
            "调用",
            "获取",
            "get",
...
__name(apply7, "apply");
var OpenAPIPluginTool = class _OpenAPIPluginTool extends import_tools7.StructuredTool {
  static {
  ...
  static fromAction(action) {
    ...
    if (Object.keys(parameters.path).length > 0) {
      const pathSchema = Object.entries(parameters.path).reduce(
        (acc, [key, param]) => {
          acc[key] = param.required ? import_zod5.z.string() : import_zod5.z.string().optional();
          return acc;
        },
        {}
      );
      schemaShape.path = import_zod5.z.object(pathSchema);
    }
    if (Object.keys(parameters.query).length > 0) {
      ...
      schemaShape.query = import_zod5.z.object(querySchema);
    }
    if (Object.keys(parameters.header).length > 0) {
      ...
      schemaShape.header = import_zod5.z.object(headerSchema);
    }
    if (Object.keys(parameters.cookie).length > 0) {
      ...
      schemaShape.cookie = import_zod5.z.object(cookieSchema);
    }
    if (Object.keys(parameters.formData).length > 0) {
      ...
      schemaShape.formData = import_zod5.z.object(formDataSchema);
    }
    if (Object.keys(parameters.body).length > 0) {
      ...
      schemaShape.body = import_zod5.z.object(bodySchema);
    }
    ...
    return new _OpenAPIPluginTool({
      ...
      schema: import_zod5.z.object(schemaShape),
      meta,
      parameters,
      requestBody,
...
function getHeadersForUrl(url, headerConfigs) {
  try {
    ...
    for (const config of headerConfigs) {
      if (import_micromatch2.default.isMatch(hostname, config.matcher, { dot: true })) {
...
async function apply8(ctx, config, plugin2) {
  ...
  const requestGetTool = new RequestsGetTool(
    plugin2,
    {
      "User-Agent": (0, import_request.randomUA)()
    },
    {
  ...
  const requestSelector = /* @__PURE__ */ __name((history) => {
    ...
    return history.some(
      (message) => message.content != null && (0, import_string7.fuzzyQuery)(
        (0, import_string7.getMessageContent)(message.content),
...
__name(apply8, "apply");
var RequestsGetTool = class extends import_tools8.StructuredTool {
  ...
  schema = import_zod6.default.object({
    url: import_zod6.default.string().describe(
...
var RequestsPostTool = class extends import_tools8.StructuredTool {
  constructor(_plugin, headers = {}, {
    maxOutputLength,
    headerConfigs,
    ...rest
  ...
  schema = import_zod6.default.object({
    ...
    data: import_zod6.default.record(import_zod6.default.any()).describe(
...
var ThinkTool = class extends import_tools9.StructuredTool {
  static {
...
11. Only set next_thought_needed to false when truly done and a satisfactory answer is reached`;
  schema = import_zod7.default.object({
    thought: import_zod7.default.string().describe("Your current thinking step"),
    nextThoughtNeeded: import_zod7.default.boolean().describe("Whether another thought step is needed"),
    thoughtNumber: import_zod7.default.number().int().min(1).describe("Current thought number"),
    totalThoughts: import_zod7.default.number().int().min(1).describe("Estimated total thoughts needed"),
    isRevision: import_zod7.default.boolean().optional().describe("Whether this revises previous thinking"),
    revisesThought: import_zod7.default.number().int().min(1).optional().describe("Which thought is being reconsidered"),
    branchFromThought: import_zod7.default.number().int().min(1).optional().describe("Branching point thought number"),
    branchId: import_zod7.default.string().optional().describe("Branch identifier"),
    needsMoreThoughts: import_zod7.default.boolean().optional().describe("If more thoughts are needed")
  });
  constructor() {
    super();
  }
...
var BuiltQuestionTool = class extends import_tools9.StructuredTool {
  static {
...
- You have only one solution option`;
  schema = import_zod7.default.object({
    question: import_zod7.default.string().describe(
      "The question or problem you want the user to choose a solution for"
    ),
    options: import_zod7.default.array(import_zod7.default.string()).min(2).max(4).describe(
...
var BuiltUserConfirmTool = class extends import_tools9.Tool {
  static {
...
var BuiltUserToastTool = class extends import_tools9.Tool {
  constructor(ctx) {
    super();
    this.ctx = ctx;
  }
...
var TodosTool = class extends import_tools10.StructuredTool {
  static {
    __name(this, "TodosTool");
  }
  name = "todos";
  schema = import_zod8.z.object({
    action: import_zod8.z.enum(["generate", "set", "get", "batch_set"]).describe("The action to perform"),
    id: import_zod8.z.string().optional().describe("The todos ID (required for set and get actions)"),
    todos: import_zod8.z.array(
      import_zod8.z.object({
        title: import_zod8.z.string().describe("The title of the todo"),
        description: import_zod8.z.string().optional().describe("The description of the todo")
      })
    ).optional().describe("The todos to generate (required for generate action)"),
    todoId: import_zod8.z.string().optional().describe(
      "The specific todo ID to set status (required for set action)"
    ),
    status: import_zod8.z.enum(["pending", "in_progress", "completed", "cancelled"]).optional().describe("The status to set (required for set action)"),
    updates: import_zod8.z.array(
      import_zod8.z.object({
        todoId: import_zod8.z.string().describe("The todo ID to update"),
        status: import_zod8.z.enum([
          "pending",
          "in_progress",
          "completed",
          "cancelled"
...
var Config2 = import_koishi2.Schema.intersect([
  ...
  import_koishi2.Schema.object({
    think: import_koishi2.Schema.boolean().default(false),
    send: import_koishi2.Schema.boolean().default(true),
    todos: import_koishi2.Schema.boolean().default(true),
    chat: import_koishi2.Schema.boolean().default(true)
  }),
  import_koishi2.Schema.object({
    draw: import_koishi2.Schema.boolean().default(false),
    music: import_koishi2.Schema.boolean().default(false)
  }),
  import_koishi2.Schema.object({
    request: import_koishi2.Schema.boolean().default(true),
    fs: import_koishi2.Schema.boolean().default(false),
    command: import_koishi2.Schema.boolean().default(false),
    cron: import_koishi2.Schema.boolean().default(true)
  }),
  import_koishi2.Schema.object({
    group: import_koishi2.Schema.boolean().default(false),
    actions: import_koishi2.Schema.boolean().default(false)
  }),
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      request: import_koishi2.Schema.const(true).required(),
      requestMaxOutputLength: import_koishi2.Schema.number().min(500).max(386e4).default(58600),
      requestSelector: import_koishi2.Schema.array(import_koishi2.Schema.string()).default([
        "请求",
        "request",
        "get",
        "post",
      ...
      requestHeaders: import_koishi2.Schema.array(
        import_koishi2.Schema.object({
          matcher: import_koishi2.Schema.string().description(
            "Domain matcher pattern (e.g., *.example.com, api.github.com)"
          ),
          headers: import_koishi2.Schema.dict(String).default({}).role("table").description("Headers to apply for this domain")
        })
      ).default([])
    }),
    import_koishi2.Schema.object({})
  ...
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      fs: import_koishi2.Schema.const(true).required(),
      fsScopePath: import_koishi2.Schema.string().default(""),
      fsSelector: import_koishi2.Schema.array(import_koishi2.Schema.string()).role("table").default([
        "文件",
        "file",
        "读取",
        "写入",
      ...
      fsIgnores: import_koishi2.Schema.array(import_koishi2.Schema.string()).role("table").default([
        "**/node_modules/**",
        "**/.git/**",
        "**/dist/**",
        "**/build/**",
  ...
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      command: import_koishi2.Schema.const(true).required(),
      commandWithSend: import_koishi2.Schema.boolean().default(true),
      commandList: import_koishi2.Schema.array(
        import_koishi2.Schema.object({
          command: import_koishi2.Schema.string(),
          description: import_koishi2.Schema.string(),
          confirm: import_koishi2.Schema.boolean().default(true),
          selector: import_koishi2.Schema.array(import_koishi2.Schema.string()).role("table").default([])
        })
      ).role("table"),
      commandBlacklist: import_koishi2.Schema.array(import_koishi2.Schema.string()).role("table").default([
        "command",
        "channel",
        "inspect",
        "plugin",
  ...
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      group: import_koishi2.Schema.const(true).required(),
      groupScopeSelector: import_koishi2.Schema.array(import_koishi2.Schema.string()),
      groupWhitelist: import_koishi2.Schema.array(import_koishi2.Schema.string()).default([])
    }),
    import_koishi2.Schema.object({})
  ]),
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      cron: import_koishi2.Schema.const(true).required(),
      cronScopeSelector: import_koishi2.Schema.array(import_koishi2.Schema.string()).default([])
    }),
    import_koishi2.Schema.object({})
  ]),
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      draw: import_koishi2.Schema.const(true).required(),
      drawPrompt: import_koishi2.Schema.string().role("textarea").default(
        // eslint-disable-next-line max-len
...
short_hair, blunt_bangs, white_hair/pink_eyes, two-tone hair, gradient hair, by Masaaki Sasamoto, best quality, masterpiece, highres, red-eyeshadow, lipstick.`
      ...
      drawCommand: import_koishi2.Schema.string().default("nai {prompt}"),
      drawSelector: import_koishi2.Schema.array(import_koishi2.Schema.string()).role("table").default([
        "画",
        "image",
        "sd",
        "图",
  ...
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      music: import_koishi2.Schema.const(true).required(),
      musicSelector: import_koishi2.Schema.array(import_koishi2.Schema.string()).role("table").default([
        "音乐",
        "music",
        "歌曲",
        "song",
  ...
  import_koishi2.Schema.union([
    import_koishi2.Schema.object({
      actions: import_koishi2.Schema.const(true).required(),
      actionsList: import_koishi2.Schema.array(
        import_koishi2.Schema.object({
          name: import_koishi2.Schema.string(),
          description: import_koishi2.Schema.string(),
          headers: import_koishi2.Schema.dict(String).default({}).role("table"),
          selector: import_koishi2.Schema.array(import_koishi2.Schema.string()).default([]).role("table"),
          openAPISpec: import_koishi2.Schema.string().role("textarea")
        })
      ).role("table")
    }),
    import_koishi2.Schema.object({})
...
function apply11(ctx, config) {
  const plugin2 = new import_chat2.ChatLunaPlugin(ctx, config, "plugin-common", false);
  logger = (0, import_logger.createLogger)(ctx, "chatluna-plugin-common");
  ctx.on("ready", async () => {
