...
import { h as h2, Service } from "koishi";
...
var AnalysisService = class extends Service {
  ...
  async ensurePersonaCache(id, defaults) {
    ...
    if (existing?.persona) {
      try {
        parsedPersona = yamlLoad(existing.persona);
      } catch (error) {
  ...
  formatMessagesForPersona(messages) {
    return messages.map((message) => {
      ...
      const normalized = normalizePersonaText(
        h2.select(message.elements, "text").map((text) => text.attrs.content).join("")
      );
        ...
        updated.push(
          h2.select(
            resolvedMessage.elements || [
              h2.text(resolvedMessage.content)
            ],
            "text"
          ).toString()
        );
        ...
        case "image":
          {
            ...
            message = typeof image === "string" ? h2.text(message) : h2.image(image, "image/png");
          }
          break;
        case "pdf": {
          ...
          message = pdfBuffer ? h2.file(pdfBuffer, "application/pdf") : h2.text("PDF 渲染失败，请检查日志。");
          break;
        }
        default: {
          message = h2.text(generateTextReport(analysisResult));
        }
      }
    } catch (error) {
  ...
  async getUserPersona(platform, selfId, userId) {
    ...
    try {
      const profile = yamlLoad(record.persona);
  ...
  async executeUserPersonaAnalysis(session, userId, force) {
    ...
    try {
      ...
      message = typeof image === "string" ? h2.text(image) : h2.image(image, "image/png");
    } catch (error) {
      this.ctx.logger.error(
...
var LLMService = class extends Service2 {
  constructor(ctx, config) {
...
import { Service as Service3 } from "koishi";
import { promises as fs } from "fs";
import path from "path";
import { fileURLToPath } from "url";
var RendererService = class extends Service3 {
  static {
  ...
  async init() {
    ...
    const tempHtmlFiles = await fs.readdir(templateDir).then(
      (files) => files.filter(
...
import { h as h4, Service as Service4 } from "koishi";
...
function CQCode(type, attrs) {
  ...
  for (const key in attrs) {
    if (attrs[key]) output += `,${key}=${h3.escape(attrs[key], true)}`;
  }
  return output + "]";
}
  ...
  function parse(source) {
    if (typeof source !== "string") {
      return source.map(({ type, data }) => {
        if (type === "text") {
          return h3("text", { content: data.text });
        } else if (type === "image") {
          return h3("img", data);
        } else if (type === "replay") {
          return h3("quote", { id: data.qq });
        } else {
          return h3(type, data);
        }
      });
    ...
    while (result = from(source)) {
      ...
      elements.push(h3(type, data));
      source = source.slice(capture.index + capture[0].length);
    }
    if (source) elements.push(h3("text", { content: unescape(source) }));
    return elements;
  }
  CQCode2.parse = parse;
  __name(parse, "parse");
...
// src/service/message.ts
var MessageService = class extends Service4 {
  constructor(ctx, config) {
    ...
    try {
      while (fetchedCount < limit) {
        ...
        const batch = messageList.data.map((msg) => ({
          ...
          elements: h4.parse(msg.content).concat(
            msg.quote ? h4(
              "quote",
              {
                id: msg.quote.id,
  ...
  async getDatabaseHistoricalMessages(filter) {
    try {
      ...
      return messages.map((message) => ({
        ...message,
        elements: h4.parse(message.content)
      }));
    } catch (error) {
      this.ctx.logger.error(
...
function apply2(ctx, config) {
  const plugin2 = new ChatLunaPlugin(
    ctx,
    config,
...
__name(apply2, "apply");
var GroupMessageFetchTool = class extends StructuredTool {
  constructor(ctx) {
    super({});
    this.ctx = ctx;
  }
  ...
  parseNaturalLanguageTime(value) {
    ...
    const parsed = parseDate(trimmed, /* @__PURE__ */ new Date());
    if (!parsed) {
      throw new Error(
...
var groupMessageFilterSchema = z.object({
  userId: z.array(z.string()).optional().describe(
...
var groupMessageFetchSchema = z.object({
  filter: groupMessageFilterSchema.optional().default({})
});
var groupUserPersonaSchema = z.object({
  user_id: z.string().describe("The target user ID whose persona should be retrieved.")
});
var GroupUserPersonaTool = class extends StructuredTool {
  constructor(ctx) {
    super({});
    this.ctx = ctx;
  }
...
var GroupListener = Schema.object({
...
var Config = Schema.intersect([
  Schema.object({
    listenerGroups: Schema.array(GroupListener).role("table").description("数据库监听规则列表。").default([]),
  ...
  Schema.object({
    alwaysPersistMessages: Schema.boolean().description(
      "启用后，无论平台能力如何都会将监听到的消息写入数据库。"
    ).default(false),
    retentionDays: Schema.number().description("数据库中缓存消息的最长保留时间（天）。").min(1).default(7)
  }).description("消息存储设置"),
  Schema.object({
    maxMessages: Schema.number().description("单次分析的最大消息数量。").default(2e3),
    minMessages: Schema.number().description("进行分析所需的最小消息数量。").min(10).max(1e3).default(100),
    ...
    wordsFilter: Schema.array(String).role("table").description("过滤词列表。消息内含有此词语时将不会记入统计消息。").default([]),
    userFilter: Schema.array(String).role("table").description("用户过滤列表。在群分析中忽略这些用户 ID 的消息。").default([]),
    personaUserFilter: Schema.array(String).role("table").description(
      "用户画像过滤列表。这些用户 ID 将无法分析用户画像（包括自动分析和手动命令调用）。"
    ).default([]),
    maxTopics: Schema.number().description("最多生成的话题数量。").default(5),
    maxUserTitles: Schema.number().description("最多生成的用户称号数量。").default(6),
    maxGoldenQuotes: Schema.number().description("最多生成的金句数量。").default(3)
  }).description("分析渲染设置"),
  Schema.object({
    model: Schema.dynamic("model").description("使用的 LLM 模型。").required(),
    temperature: Schema.number().description("生成的温度。").min(0).max(2).default(1.5)
  }).description("LLM 设置"),
  Schema.object({
    personaAnalysisMessageInterval: Schema.number().description(
      "跨群用户画像分析的触发阈值，新消息累计达到该条数时尝试更新画像。设置为 0 则关闭自动画像分析。"
    ).min(0).default(50),
    personaCacheLifetimeDays: Schema.number().description(
      "用户画像缓存结果的保留天数。超过此时长后再次请求会重新生成画像。"
    ).min(0).default(3),
    personaLookbackDays: Schema.number().description("画像分析回溯的天数窗口（建议保持在 1-4 天）。").min(1).max(7).default(2),
    personaMaxMessages: Schema.number().description("单次用户画像分析最多提取的历史消息数量。").min(100).max(1500).default(400),
    personaMinMessages: Schema.number().description("触发用户画像分析所需的最少历史消息数量。").min(10).default(20)
  }).description("用户画像设置"),
  Schema.object({
...
\`\`\``
    ...
    promptUserPersona: Schema.string().description("跨群用户画像分析的提示词模板。").role("textarea").default(
