...
import WebSocket$1, { CloseEvent, ErrorEvent } from "ws";
import E3 from "eventemitter3";

//#region src/base/buffer.d.ts
/**
* 消息操作类型
*/
/**
 ...
 * 协议版本
 */
declare enum WS_BODY_PROTOCOL_VERSION {
  NORMAL = 0,
  NUMBER = 1,
  ZLIB = 2,
  BROTLI = 3,
...
declare function serialize(type: WS_OP, data?: string | Record<string, any>): Uint8Array; //#endregion
//#region src/base/types.d.ts
type BILIBILI_HOST = string;
type PartialByKeys<O extends Record<string, any>, U = keyof O, UU = (U extends keyof O ? U : never), PO = Partial<Pick<O, UU>>> = PO & { [K in keyof O as K extends U ? never : K]: O[K] };
type Merge<A, B> = { [K in keyof A | keyof B]: K extends keyof A & keyof B ? A[K] | B[K] : K extends keyof B ? B[K] : K extends keyof A ? A[K] : never };
...
interface BaseLiveClientOptions<B extends Uint8Array> extends Options {
...
interface WSOptions extends BaseOptions, AuthOptions, ConnectionOptions {
  headers?: {
    [key: string]: string;
  } | undefined;
  ssl?: boolean;
...
interface TCPOptions extends WSOptions {}
interface MessageMeta extends ProtocolHeader {}
interface Message<T> {
  meta: MessageMeta;
  data: T;
}
interface IZlib<T extends Uint8Array = Uint8Array> {
  inflateAsync: (v: T) => Promise<T>;
  brotliDecompressAsync: (v: T) => Promise<T>;
} //#endregion
//#region src/base/cmd.d.ts
interface DANMU_MSG {
  cmd: 'DANMU_MSG';
  info: [[number, number, number, number, number, number, number, string, number, number, number, string, number, string, string, {
    mode: number;
    ...
    not_show: number;
  }], string, [number, string, number, number, number, number, number, string], [number, string, string, number, number, string, number, number, number, number, number, number, number], [number, number, number, string, number], [string, string], number, number, NonNullable<any>, {
    ts: number;
    ct: string;
  }, 0, 0, NonNullable<any>, NonNullable<any>, number, number];
  /** Base64 String */
  dm_v2: string;
}
interface SEND_GIFT {
...
interface SUPER_CHAT_MESSAGE_JPN {
...
interface ONLINE_RANK_COUNT {
  cmd: 'ONLINE_RANK_COUNT';
  data: {
    count: number;
  };
}
interface STOP_LIVE_ROOM_LIST {
...
interface SUPER_CHAT_MESSAGE_DELETE {
...
interface POPULARITY_RED_POCKET_NEW {
...
interface RED_POCKET_GIFT {
  gift_id: number;
  gift_name: string;
  gift_pic: string;
  num: number;
}
interface RED_POCKET_GIFT_WINNER {
  uid: number;
  name: string;
  user_type: number;
  award_type: number;
...
interface POPULARITY_RED_POCKET_START {
...
interface POPULARITY_RED_POCKET_WINNER_LIST {
...
interface ROOM_ADMIN_REVOKE {
  cmd: 'ROOM_ADMIN_REVOKE';
  msg: string;
  uid: number;
}
interface ONLINE_RANK_V2 {
  cmd: 'ONLINE_RANK_V2';
  data: {
    list: ({
      uid: number;
...
interface HOT_RANK_SETTLEMENT_V2 {
...
interface ANCHOR_LOT_CHECKSTATUS {
...
interface ANCHOR_LOT_END {
  cmd: 'ANCHOR_LOT_END';
  data: {
    id: number;
  };
}
interface ANCHOR_LOT_AWARD {
...
interface VIDEO_CONNECTION_JOIN_START {
...
interface VIDEO_CONNECTION_MSG {
...
interface VIDEO_CONNECTION_JOIN_END {
...
interface FULL_SCREEN_SPECIAL_EFFECT {
...
interface COMMON_NOTICE_DANMAKU {
...
interface LIKE_INFO_V3_UPDATE {
  cmd: 'LIKE_INFO_V3_UPDATE';
  data: {
    click_count: number;
  };
}
interface LIKE_INFO_V3_CLICK {
...
interface POPULAR_RANK_CHANGED {
...
interface LIVE_INTERACTIVE_GAME {
...
interface MULTI_VOICE_OPERATIN {
...
interface MULTI_VOICE_APPLICATION_USER {
...
interface GOTO_BUY_FLOW {
  cmd: 'GOTO_BUY_FLOW';
  data: {
    text: string;
  };
}
interface GUARD_HONOR_THOUSAND {
...
interface RECOMMEND_CARD_LIST_ITEM {
...
interface SHOPPING_CART_SHOW {
  cmd: 'SHOPPING_CART_SHOW';
  data: {
    status: number;
  };
}
interface TRADING_SCORE {
  cmd: 'TRADING_SCORE';
  data: {
...
interface VOICE_JOIN_ROOM_COUNT_INFO {
...
interface WIDGET_GIFT_STAR_PROCESS {
...
interface LIVE_MULTI_VIEW_CHANGE {
...
interface PK_BATTLE_FINAL_PROCESS {
...
interface PK_BATTLE_PROCESS_NEW {
...
interface ACTIVITY_BANNER_CHANGE_V2 {
...
interface ACTIVITY_BANNER_CHANGE {
...
type ReplaceVToMessage<E extends Record<string, any>> = { [K in keyof E]: Message<E[K]> };
type BuiltinEvent = ReplaceVToMessage<_BuiltinEvent>; //#endregion
//#region src/base/eventemitter.d.ts
type Nil = undefined | void | null;
type EventKey = string | symbol;
type ListenerResult = void | Promise<void>;
type Listener<O extends Record<EventKey, any>, K extends keyof O, V = O[K]> = V extends Array<any> ? V extends [infer Arg] ? (arg1: Arg) => ListenerResult : V extends [infer Arg1, infer Arg2] ? (arg1: Arg1, arg2: Arg2) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3, infer Arg4] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3, arg4: Arg4) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3, infer Arg4, infer Arg5] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3, arg4: Arg4, arg5: Arg5) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3, infer Arg4, infer Arg5, infer Arg6] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3, arg4: Arg4, arg5: Arg5, arg6: Arg6) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3, infer Arg4, infer Arg5, infer Arg6, infer Arg7] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3, arg4: Arg4, arg5: Arg5, arg6: Arg6, arg7: Arg7) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3, infer Arg4, infer Arg5, infer Arg6, infer Arg7, infer Arg8] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3, arg4: Arg4, arg5: Arg5, arg6: Arg6, arg7: Arg7, arg8: Arg8) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3, infer Arg4, infer Arg5, infer Arg6, infer Arg7, infer Arg8, infer Arg9] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3, arg4: Arg4, arg5: Arg5, arg6: Arg6, arg7: Arg7, arg8: Arg8, arg9: Arg9) => ListenerResult : V extends [infer Arg1, infer Arg2, infer Arg3, infer Arg4, infer Arg5, infer Arg6, infer Arg7, infer Arg8, infer Arg9, infer Arg10] ? (arg1: Arg1, arg2: Arg2, arg3: Arg3, arg4: Arg4, arg5: Arg5, arg6: Arg6, arg7: Arg7, arg8: Arg8, arg9: Arg9, arg10: Arg10) => ListenerResult : (...args: any[]) => ListenerResult : V extends Nil ? () => ListenerResult : V extends boolean ? (arg: boolean) => ListenerResult : (arg: V) => ListenerResult;
declare class EventEmitter<E extends Record<EventKey, any>> extends E3 {
  on<N extends keyof E>(eventName: N, listener: Listener<E, N>): this;
  once<N extends keyof E>(eventName: N, listener: Listener<E, N>): this;
  addListener<N extends keyof E>(eventName: N, listener: Listener<E, N>): this;
  off<N extends keyof E>(eventName: N, listener: Listener<E, N>): this;
  emit<N extends keyof E>(eventName: N, ...args: Parameters<Listener<E, N>>): boolean;
  removeListener<N extends keyof E>(eventName: N, listener: Listener<E, N>): this;
  removeAllListeners<N extends keyof E>(eventName?: N): this;
  listenerCount<N extends keyof E>(eventName?: N): number;
}

//#endregion
//#region src/base/base.d.ts
declare enum SOCKET_HOSTS {
  DEFAULT = "zj-cn-live-comet.chat.bilibili.com",
}
declare const WEBSOCKET_SSL_URL: (host?: BILIBILI_HOST, port?: number, path?: string) => string;
declare const WEBSOCKET_URL: (host?: BILIBILI_HOST, port?: number, path?: string) => string;
...
declare class LiveClient<E extends Record<EventKey, any>> extends EventEmitter<Merge<BilibiliLiveEvent, E>> {
  readonly options: BaseLiveClientOptions<any>;
  /** 人气值 */
  online: number;
  closed: boolean;
  roomId: number;
  private close_func_called;
  private socket;
  private timeout;
  private readonly HEARTBEAT_TIME;
  private readonly RECONNECT_TIME;
  private zlib;
  private live;
  private skipMessage;
  constructor(options: BaseLiveClientOptions<any>);
  onlyListener(events: string[] | string): void;
  clearOnlyListener(events: string[] | string): void;
  private rawSend;
  private bindEvent;
  send(op: WS_OP, data?: Record<string, any> | string): void;
  runWhenConnected(...fns: (() => void)[]): Promise<void>;
  heartbeat(): void;
...
declare class KeepLiveTCP<E extends Record<EventKey, any> = object> extends LiveClient<Merge<TCPEvents, E>> {
  private buffer;
  private i;
  tcpSocket: Socket;
...
declare class KeepLiveWS<E extends Record<EventKey, any> = object> extends LiveClient<Merge<WSEvents, E>> {
  ws: WebSocket$1;
...
declare function deserialize(buffer: Uint8Array): Promise<Message<any>[]>;

//#endregion
//#region src/node/api.d.ts
declare function getLongRoomId(room: number, headers?: Record<string, string>): Promise<RoomResponse>;
declare function getCachedInfo(room: number, options: BaseLiveClientOptions<any>): Promise<[RoomResponse, DanmuConfResponse, string]>;

//#endregion
//#region src/base/utils.d.ts
declare function fromEvent<T>(emitter: EventEmitter<any>, event: string, timeout?: number): Promise<T>;
declare function toMessageData(message: Message<any>): any;

//#endregion
export { ACTIVITY_BANNER_CHANGE, ACTIVITY_BANNER_CHANGE_V2, ANCHOR_LOT_AWARD, ANCHOR_LOT_CHECKSTATUS, ANCHOR_LOT_END, ANCHOR_LOT_START, AREA_RANK_CHANGED, BuiltinEvent, CHANGE_ROOM_INFO, COMBO_SEND, COMMON_NOTICE_DANMAKU, CUT_OFF, DANMU_AGGREGATION, DANMU_MSG, DANMU_TAG_CHANGE, DM_INTERACTION, ENTRY_EFFECT, EventEmitter, EventKey, FULL_SCREEN_SPECIAL_EFFECT, GIFT_STAR_PROCESS, GOTO_BUY_FLOW, GUARD_BUY, GUARD_HONOR_THOUSAND, HOT_RANK_CHANGED, HOT_RANK_CHANGED_V2, HOT_RANK_SETTLEMENT, HOT_RANK_SETTLEMENT_V2, HOT_ROOM_NOTIFY, INTERACT_WORD, KeepLiveTCP, KeepLiveWS, LIKE_INFO_V3_CLICK, LIKE_INFO_V3_UPDATE, LIVE, LIVE_INTERACTIVE_GAME, LIVE_MULTI_VIEW_CHANGE, Listener, MULTI_VOICE_APPLICATION_USER, MULTI_VOICE_OPERATIN, MedalInfo, Message, MessageMeta, NOTICE_MSG, ONLINE_RANK_COUNT, ONLINE_RANK_TOP3, ONLINE_RANK_V2, PK_BATTLE_END, PK_BATTLE_FINAL_PROCESS, PK_BATTLE_PRE, PK_BATTLE_PRE_NEW, PK_BATTLE_PROCESS, PK_BATTLE_PROCESS_NEW, PK_BATTLE_SETTLE, PK_BATTLE_SETTLE_USER, PK_BATTLE_SETTLE_V2, PK_BATTLE_START, PK_BATTLE_START_NEW, PLAY_TAG, POPULARITY_RED_POCKET_NEW, POPULARITY_RED_POCKET_START, POPULARITY_RED_POCKET_WINNER_LIST, POPULAR_RANK_CHANGED, PREPARING, Protocol, ProtocolHeader, RANK_REM, RECOMMEND_CARD, RECOMMEND_CARD_LIST_ITEM, RED_POCKET_GIFT, RED_POCKET_GIFT_WINNER, ROOM_ADMINS, ROOM_ADMIN_REVOKE, ROOM_BLOCK_MSG, ROOM_CHANGE, ROOM_REAL_TIME_MESSAGE_UPDATE, ROOM_SILENT_OFF, ROOM_SILENT_ON, ROOM_SKIN_MSG, ReplaceVToMessage, RoomResponse, SEND_GIFT, SHOPPING_CART_SHOW, SOCKET_HOSTS, SPECIAL_GIFT, STOP_LIVE_ROOM_LIST, SUPER_CHAT_MESSAGE, SUPER_CHAT_MESSAGE_DELETE, SUPER_CHAT_MESSAGE_JPN, SYS_MSG, TCPOptions, TRADING_SCORE, USER_TOAST_MSG, VIDEO_CONNECTION_JOIN_END, VIDEO_CONNECTION_JOIN_START, VIDEO_CONNECTION_MSG, VOICE_JOIN_LIST, VOICE_JOIN_ROOM_COUNT_INFO, VOICE_JOIN_STATUS, VOICE_JOIN_SWITCH, WARNING, WATCHED_CHANGE, WEBSOCKET_SSL_URL, WEBSOCKET_URL, WIDGET_BANNER, WIDGET_GIFT_STAR_PROCESS, WIDGET_WISH_LIST, WSOptions, WS_BODY_PROTOCOL_VERSION, WS_OP, deserialize, fromEvent, getCachedInfo, getLongRoomId, room_admin_entrance, serialize, toMessageData };