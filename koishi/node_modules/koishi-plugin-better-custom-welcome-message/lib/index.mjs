...
var Config = Schema.intersect([
  Schema.object({
    welcome_group_selector: Schema.natural().computed().default(0).description("欢迎消息选择器"),
    leave_group_selector: Schema.natural().computed().default(0).description("离开消息选择器")
  }).description("群组配置"),
  Schema.object({
    custom_welcome_messages: Schema.array(Schema.string().role("textarea")).default([`{at} 欢迎 {user} 加入 {group}，送你一句话吧：{hitokoto}`]).description("欢迎消息"),
...
function transformElements(session, element, parent) {
  ...
  const resultElement = h.jsx(element.type, element.attrs);
